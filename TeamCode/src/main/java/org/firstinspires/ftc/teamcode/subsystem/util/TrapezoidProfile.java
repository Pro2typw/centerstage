...
public class TrapezoidProfile {
    ...
    public TrapezoidProfile(Constraints constraints, State goal, State initial) {
        ...
        this.constraints = constraints;
        current = direct(initial);
        this.goal = direct(goal);
        if (current.velocity > this.constraints.maxVelocity) {
            current.velocity = this.constraints.maxVelocity;
        }

        // Deal with a possibly truncated motion profile (with nonzero initial or
        // final velocity) by calculating the parameters as if the profile began and
        // ended at zero velocity
        double cutoffBegin = current.velocity / this.constraints.maxAcceleration;
        ...
        double fullTrapezoidDist =
                cutoffDistBegin + (this.goal.position - current.position) + cutoffDistEnd;
        double accelerationTime = this.constraints.maxVelocity / this.constraints.maxAcceleration;

        double fullSpeedDist =
                fullTrapezoidDist - accelerationTime * accelerationTime * this.constraints.maxAcceleration;
    ...
    public State calculate(double t) {
        ...
        if (t < endAccel) {
            ...
            result.position +=
                    (current.velocity + endAccel * constraints.maxAcceleration / 2.0) * endAccel
            ...
            result.position =
                    goal.position
                            - (goal.velocity + timeLeft * constraints.maxAcceleration / 2.0) * timeLeft;
        } else {
            result = goal;
        }
    ...
    public State calculate(double t, State current, State goal) {
        ...
        if (this.current.velocity > constraints.maxVelocity) {
            this.current.velocity = constraints.maxVelocity;
        }

        // Deal with a possibly truncated motion profile (with nonzero initial or
        // final velocity) by calculating the parameters as if the profile began and
        // ended at zero velocity
        double cutoffBegin = this.current.velocity / constraints.maxAcceleration;
        ...
        double fullTrapezoidDist =
                cutoffDistBegin + (goal.position - this.current.position) + cutoffDistEnd;
        ...
        endDeccel = endFullSpeed + accelerationTime - cutoffEnd;
        State result = new State(this.current.position, this.current.velocity);

        if (t < endAccel) {
            result.velocity += t * constraints.maxAcceleration;
            result.position += (this.current.velocity + t * constraints.maxAcceleration / 2.0) * t;
        } else if (t < endFullSpeed) {
            result.velocity = constraints.maxVelocity;
            result.position +=
                    (this.current.velocity + endAccel * constraints.maxAcceleration / 2.0) * endAccel
                            + constraints.maxVelocity * (t - endAccel);
        } else if (t <= endDeccel) {
            result.velocity = goal.velocity + (endDeccel - t) * constraints.maxAcceleration;
            double timeLeft = endDeccel - t;
    ...
    public double timeLeftUntil(double target) {
        double position = current.position * direction;
        double velocity = current.velocity * direction;
        ...
        final double acceleration = constraints.maxAcceleration;
        final double decceleration = -constraints.maxAcceleration;
