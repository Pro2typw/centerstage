...
public class RedBackdropPropDetection implements VisionProcessor {
    ...
    public Object processFrame(Mat frame, long captureTimeNanos) {
        ...
        if(biggestContour != null) {
            ...
            Imgproc.rectangle(frame, RIGHT_ROI, new Scalar(255, 0, 0));
            Imgproc.rectangle(frame, CENTER_ROI, new Scalar(255, 0, 0));
            
            double rightArea = calculateIntersectionArea(RIGHT_ROI, rect) / RIGHT_ROI.area();
            double centerArea = calculateIntersectionArea(CENTER_ROI, rect) / CENTER_ROI.area();
            
            if(Math.max(rightArea, centerArea) < expectedPercent) location = AllianceStates.LEFT;
            else if(rightArea < centerArea) location = AllianceStates.CENTER;
            else location = AllianceStates.RIGHT;
