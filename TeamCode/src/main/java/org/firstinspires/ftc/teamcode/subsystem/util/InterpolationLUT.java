...
public class InterpolationLUT {
    ...
    private InterpolationLUT(List<Double> x, List<Double> y, List<Double> m) {
        mX = x;
        mY = y;
        mM = m;
    }

    public InterpolationLUT() {
    }
    ...
    public void createLUT() {
        ...
        if (x == null || y == null) throw new IllegalArgumentException("The lists can't be null");
        if(x.size() != y.size()) throw new IllegalArgumentException("The arrays must be of equal length.");
        if( x.size() < 2) throw new IllegalArgumentException("The arrays has fewer than 2 points.");
