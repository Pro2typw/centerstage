# Notiec

urmom
w/
me
in
the
bed

## Goals
- U League Meet 3
  - <span style="color:yellow">Arm Differential (code done, but need to tune pid and test whether difference is extension or vice versa)</span>
  - <span style="color:green">RR Pathing (finished)</span>
  - <span style="color:yellow">RR Tuning (not starty)</span>
  - <span style="color:yellow">Heading for IMU (code done, needs robo to be built)</span>
  - <span style="color:yellow">April Tag Localization need testing (math incorrect, fix)</span>
  - <span style="color:yellow">Prop Detection (code done, tune ROI)</span>
- U League Tournament
  - Extension Motion Profiling
  - Wrist Differential
  - Max Velocity/Acceleration Auto tuner (lowkey sounding like a regionals thing to do (if needed))
- Regionals
- States
- We go worlds
- MIT
  - Hang PID (code done, lowkey sounding like a regionals thing to do)
